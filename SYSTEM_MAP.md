...
**Status:** Canonical production architecture after cleanup, regional/city expansion, deployment-path verification, NHS Administrative & Clerical integration, search/UX hardening, Customer Sales production launch and three-family live/diagnostic regional reporting with rolling history.
...
The daily coverage pass reuses the same materialized JobG8 workbook and governed family rules across all 33 canonical regions. It persists the current Service Admin, Support Worker and Customer Sales diagnostic counts in `pipeline/reports-daily/daily-family-coverage.csv`. It also records the feed date in `pipeline/reports-daily/daily-family-coverage-history.json`: all 99 region/family counts are stored, same-date reruns replace the prior snapshot, and only the latest 14 feed dates are retained. The history begins on 22 August 2026 with no backfill. For Customer Sales, the diagnostic path mirrors the production classifier, canonical geo, campaign dedupe and final QA without writing public slice output. Coverage and history are evidence only and never activate slices.
...
- `pipeline/reports-daily/daily-family-coverage.csv` — current same-feed Service Admin, Support Worker and Customer Sales coverage across all 33 canonical regions (99 region/family rows);
- `pipeline/reports-daily/daily-family-coverage-history.json` — rolling latest-14-feed-date history of all 99 diagnostic counts; one snapshot per feed date, with same-date reruns replacing rather than duplicating;
...
The rolling family history begins on **22 August 2026** with no backfill. All 99 counts are stored even though rolling metrics are displayed only for NOT LIVE slices, so evidence remains continuous if a slice later changes LIVE state. The `6+ days` metric is a simple consistency/watch signal and is not an automatic publish threshold.
