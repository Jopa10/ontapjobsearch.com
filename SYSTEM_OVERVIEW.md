...
- 21 August 2026 — Customer Sales / Sales Advisor has moved into proof-region testing only. Hampshire, Manchester & Salford and West Yorkshire are the agreed first test slices on `agent/customer-sales-family`; genuine Sales/Service Admin overlap remains valid family membership. Nothing in this checkpoint is production-approved or LIVE.
...
Customer Sales / Sales Advisor is currently at the **proof-region test** stage. The agreed first test slices are **Hampshire**, **Manchester & Salford**, and **West Yorkshire**, built only on `agent/customer-sales-family` so the actual jobs, employer spread and page quality can be inspected. Genuine Sales/Service Admin crossover jobs remain eligible for Customer Sales when they genuinely serve both user intents. The family and these three slices remain non-production: no LIVE approval, no production pipeline/config change and no recurring 33-region Sales assessment has yet been authorised.
...
Search uses a compact deployment-time snapshot of current published inventory. `scripts/generate-search-index.ts` creates `generated/published-jobs-search.json` during `npm run build`; `/jobs/search` imports that compact index directly rather than walking/parsing the full published route-data tree for each request. This removes the expensive request-time inventory reconstruction that was causing noticeable waits, while keeping the search data tied to the deployed published supply.
...
High-confidence spelling correction is applied from the current published-job vocabulary, with ambiguous/tied corrections left unchanged. Both search inputs also enable browser spellcheck/autocorrect. Correction vocabularies and recent correction results are cached, and the dynamic search route prefers Vercel London (`lhr1`) for UK requests.

Production verification on 21 August includes `lumley office` → Great Lumley Surgery, `lumley offcie` → corrected `lumley office` → the same vacancy, and `admin` + `newcastle` returning 21 current matches whether the role/location values are entered in the normal or reversed boxes.
...
- the twice-daily full JobG8 process, which now refreshes and composes NHS transactionally inside the Service Admin path;
...
Every `npm run build` now regenerates the compact published-job search index before `prisma generate` and `next build`, so search always uses the published inventory captured by that deployment without rebuilding the inventory on individual search requests.
