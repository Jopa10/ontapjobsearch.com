...
Customer Sales uses the same dynamic configured-slice mechanism. Once the verified publisher writes non-empty data, the approved routes are `/job-search/london/customer-sales-jobs`, `/job-search/west-yorkshire/customer-sales-jobs`, and `/job-search/manchester-salford/customer-sales-jobs`; the same published-job inventory feeds Browse Jobs, homepage discovery, sitemap and search indexing.
