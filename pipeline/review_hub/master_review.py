...
from .nhs_decisions import write_decision as write_nhs_decision
...
def master_text(results: Iterable[SourceResult], *, today: date, previous: Path | None = None) -> str:
    ...
    for result in results:
        ...
        if not result.items:
            ...
            continue
        for item in sorted(result.items, key=lambda x: (x.region.casefold(), x.location.casefold(), x.title.casefold(), x.source_job_id)):
            action = carried.get(_item_key(result.key, item), "")
            headline = " | ".join(("POSS", _md(result.label), _md(item.region) or "—", _md(item.location) or "—", _md(item.salary) or "—", _md(item.title)))
            lines.extend([
                "---",
...
def build_master(path: Path = DEFAULT_MASTER, *, today: date | None = None) -> dict[str, object]:
    ...
    path.write_text(master_text(results, today=today, previous=path), encoding="utf-8")
    return {
...
def parse_master(path: Path = DEFAULT_MASTER) -> tuple[str, list[ParsedDecision]]:
    ...
    date_match = re.search(r"(?m)^review_date:[ \t]*(\d{4}-\d{2}-\d{2})[ \t]*$", text)
    if not date_match:
    ...
    for block in _blocks(text):
        ...
        item = ReviewItem(
            source=_field(block, "source"), source_job_id=source_job_id,
            title=_field(block, "title"), employer=_field(block, "employer"),
            location=_field(block, "location"), region=_field(block, "region"),
            category=_field(block, "category"), salary=_field(block, "salary"),
            closing_date=_field(block, "closing_date"), reason=_field(block, "reason"),
            source_url=_field(block, "source_url"),
        )
        fingerprint = _field(block, "hub_fingerprint")
        if not item.title or not fingerprint:
            raise ValueError(f"incomplete master review facts for {source_key}/{source_job_id}")
...
def _patch_action(path: Path, id_field: str, source_job_id: str, action: str) -> None:
    ...
    def replace(match: re.Match[str]) -> str:
        nonlocal matched
        block = match.group(1)
        id_match = re.search(rf"(?mi)^{re.escape(id_field)}:[ \t]*(\S+)[ \t]*$", block)
        if not id_match or clean(id_match.group(1)) != source_job_id:
            return match.group(0)
        if not re.search(r"(?mi)^action:[ \t]*(?:select|exclude)?[ \t]*$", block):
            raise ValueError(f"review block has no editable action line: {source_job_id}")
        matched += 1
        block = re.sub(r"(?mi)^action:[ \t]*(?:select|exclude)?[ \t]*$", f"action: {action}", block, count=1)
        return f"---\n{block}---"
...
def _route_action(decision: ParsedDecision) -> None:
    if decision.source_key == "jobg8":
        ...
        return
    if decision.source_key == "nhs":
        write_nhs_decision(
            PIPELINE_ROOT / "reviews/external/nhs-jobs-decisions.csv",
            category=decision.item.category,
            source_job_id=decision.item.source_job_id,
            hub_fingerprint=decision.fingerprint,
            action=decision.action,
            title=decision.item.title,
        )
        return
    routes = {
        "nejobs": (PIPELINE_ROOT / "reviews/external/northeast-jobs-summary.md", "source_job_id"),
        "vonne": (PIPELINE_ROOT / "reviews/external/vonne-summary.md", "source_job_id"),
        "teaching_vacancies": (PIPELINE_ROOT / "reviews/external/teaching-vacancies/england-wide-admin-service-summary.md", "source_job_id"),
    }
...
def apply_master(path: Path = DEFAULT_MASTER, *, today: date | None = None, write: bool = False, plan_path: Path | None = None, require_complete: bool = False) -> dict[str, object]:
    today = today or date.today()
    ...
    if require_complete:
        ...
        if blank:
            examples = ", ".join(f"{decision.source_key}/{decision.item.source_job_id}" for decision in blank[:6])
            suffix = " ..." if len(blank) > 6 else ""
            raise ValueError(f"{len(blank)} review item(s) still have blank action lines: {examples}{suffix}")
    results = load_all_sources(today)
    by_key = {result.key: result for result in results}
    current = {
        (result.key, item.category, item.source_job_id): item
        for result in results if result.state == "OK" for item in result.items
    }
    ...
    for decision in acted:
        ...
        if decision.fingerprint != live_item.fingerprint():
            raise ValueError("vacancy facts changed since review for " f"{decision.source_key}/{decision.item.source_job_id}")
    if write:
        for decision in acted:
            _route_action(decision)
    publish_sources = [result for result in results if result.state == "OK" and result.publish_workflow]
    plan = {
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if args.command == "build":
            ...
            report = apply_master(args.input, write=args.write, plan_path=args.plan, require_complete=args.require_complete)
