...
def main() -> int:
    ap = argparse.ArgumentParser(
        description="Assess Claims Support regional recurrence across recent archived JobG8 feed dates."
    )
            ...
            daily_rows.append(
                {
                    "feed_date": feed_date,
                    "region": region,
                    "claims_support_count": counts.get(region, 0),
                }
            )
        ...
        summary_rows.append(
            {
                ...
                "average_count": round(sum(values) / len(values), 2),
                "median_count": statistics.median(values),
                "minimum_count": min(values),
                "maximum_count": max(values),
                "days_with_1_plus": sum(value >= 1 for value in values),
                "days_with_4_plus": sum(value >= 4 for value in values),
                "days_with_6_plus": sum(value >= 6 for value in values),
                "recent_counts": " / ".join(str(value) for value in values),
                "evidence_grade": evidence_grade(values),
            }
        )

    summary = pd.DataFrame(summary_rows).sort_values(
        by=["latest_count", "average_count", "region"],
    ...
    lines = [
        ...
        "",
        f"Strong review candidates: **{len(strong)}**.",
        f"Watch regions: **{len(watch)}**.",
        "",
        "## Regional evidence",
        "",
    ...
    for _, row in summary.iterrows():
        lines.append(
            f"| {str(row['region']).replace('|', '/')} | {int(row['latest_count'])} | "
            f"{float(row['average_count']):.2f} | {float(row['median_count']):.1f} | "
            f"{int(row['days_with_6_plus'])} | {row['recent_counts']} | {row['evidence_grade']} |"
        )
