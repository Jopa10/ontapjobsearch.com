...
# Four advert patterns remained genuinely arguable in the manual pass.
...
def boundary_decision(title: str, description: str, annual_max: float | None) -> tuple[str, str]:
    ...
    # User rule: salaries over £50k are automatically outside this Ontap family.
...
def main() -> int:
        ...
        rows.append(
            {
                ...
                "title": title,
                "location": location,
                "ontap_region": ontap_region(
                    row.get(AREA_COL, ""), row.get(LOCATION_COL, ""), area_lookup, fallback
                ),
    ...
    region_counts = unique_in.loc[
        unique_in["ontap_region"] != "Other / Unknown", "ontap_region"
    ].value_counts()
    ...
    lines = [
        ...
        f"Broad insurance/claims universe: **{len(out):,}** raw rows",
        f"IN after advert-level boundary rules: **{len(in_rows):,}** raw rows",
        f"BORDERLINE: **{len(borderline):,}** raw rows",
        f"OUT: **{counts.get('OUT', 0):,}** raw rows",
    ...
    lines += [
        ...
        "## Salary guard check",
        "",
        f"Content-unique IN jobs with usable annualised maximum: **{len(salary_max):,}**.",
        f"IN jobs still over £50k: **{over_50}** (must be zero).",
    ]
