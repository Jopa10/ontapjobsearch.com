"""Run support-worker using every LIVE support slice from the slice register."""
...
def write_manual_review_markdown(
    ...
    base_action_rows = [
        ...
        if str(row.get("region", "")) not in extras
    ]
    ...
    if marker not in content:
        raise RuntimeError("Support-worker review Markdown is missing the active-actions marker")
