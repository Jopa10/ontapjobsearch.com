...
SOURCE_CODE = "BVSC"
LIST_URL = (
    "https://www.bvsc.org/pages/site/"
    "bvsc-charity-jobs-wm/category/vacancies"
)
USER_AGENT = (
...
GENERIC_LOCATION_MARKERS = (
    "hybrid", "home based", "home-based", "remote", "west midlands",
)
...
def parse_listing(text: str) -> list[ListingItem]:
    ...
    for title, href in parser.links:
        ...
        if not title or absolute in seen:
            continue
        if "/vacancy" not in path and not any(
            word in path
            for word in ("assistant", "officer", "administrator", "reception")
        ):
            continue
        seen.add(absolute)
...
def parse_detail(text: str, item: ListingItem) -> dict[str, str]:
    ...
    closing = _field(
        lines,
        ("Closing date for applications", "Closing Date", "Application deadline"),
    ) or item.closing_date
    employer = item.employer
    if not employer:
        heading = next(
            (line for line in lines if "," in line and "closing date" in line.casefold()),
            "",
        )
...
def classify(vacancy: BvscVacancy) -> tuple[str, str]:
    ...
    if clear:
        if any(p in context for p in STRONG_SPECIALIST_PATTERNS) and not any(
            p in title for p in PRIMARY_CLEAR_PATTERNS
        ):
    ...
    if any(marker in context for marker in ("administrative", "customer service", "office support")):
...
def duplicate_against(
    ...
    return (
        status,
        _candidate_value(best_candidate, "source"),
        _candidate_value(best_candidate, "title"),
        _candidate_value(best_candidate, "employer"),
        f"{best_score:.3f}",
    )
...
def report_row(v: BvscVacancy) -> dict[str, str]:
    return {
        "source": SOURCE_NAME,
        "tracking_key": f"bvsc-{v.source_job_id}",
        "title": v.title,
        "employer": v.employer,
        "location": v.location,
        "salary_text": v.salary_text,
        "closing_date": v.closing_date,
        "description_excerpt": v.description_excerpt,
        "apply_url": v.apply_url,
        "classification": v.classification,
        "classification_reason": v.classification_reason,
        "geography_status": v.geography_status,
        "geography_reason": v.geography_reason,
        "ontap_geography": v.ontap_geography,
        "jobg8_check": v.duplicate_status,
        "jobg8_candidate_title": v.jobg8_candidate_title,
        "jobg8_candidate_employer": v.jobg8_candidate_advertiser,
        "jobg8_match_score": v.jobg8_match_score,
        "external_check": v.external_duplicate_status,
        "external_candidate_source": v.external_candidate_source,
        "external_candidate_title": v.external_candidate_title,
        "external_candidate_employer": v.external_candidate_employer,
        "external_match_score": v.external_match_score,
        "bvsc_duplicate_check": v.source_duplicate_status,
...
def write_reports(vacancies: list[BvscVacancy], csv_path: Path, md_path: Path) -> None:
    ...
    counts = {
        key: sum(v.classification == key for v in vacancies)
        for key in ("HC", "POSS", "HARD_PASS")
    }
    lines = [
    ...
    for v in vacancies:
        lines += [
            f"### [{v.classification}] {v.title}",
            f"- Employer: {v.employer or 'Not stated'}",
            f"- Location: {v.location or 'Not stated'}",
            f"- Salary: {v.salary_text or 'Not stated'}",
            f"- Closing: {v.closing_date or 'Not stated/open-ended'}",
            f"- Reason: {v.classification_reason}",
            f"- JobG8: {v.duplicate_status or 'NO_MATCH'}",
            f"- External: {v.external_duplicate_status or 'NO_MATCH'}",
            f"- Source: {v.source_url}",
            "- action:",
            "",
        ]
...
def _load_jobg8_for_bvsc(jobg8_path: Path, geo_lookup_path: Path) -> list[object]:
    if not jobg8_path.exists():
        return []
    area_map, fallback_map = load_geo_lookup(geo_lookup_path)
    return load_jobg8_candidates(jobg8_path, area_map, fallback_map)
...
def run(args: argparse.Namespace) -> list[BvscVacancy]:
    listing_text = (
        fetch_text(LIST_URL)
        if args.fetch_live
        else Path(args.listing_file).read_text(encoding="utf-8")
    )
    ...
    for item in items:
        ...
        if details_dir:
            for suffix in (".html", ".txt", ".md"):
                ...
                if path.exists():
                    detail_text = path.read_text(encoding="utf-8")
                    status = "snapshot"
                    break
        elif args.fetch_live:
            try:
                detail_text = fetch_text(item.source_url)
                status = "live"
        ...
        geo_status, geo_reason = geography(
            detail["location"],
        ...
        vacancy = BvscVacancy(
            ...
            description_excerpt=detail["description_excerpt"],
    ...
    jobg8 = _load_jobg8_for_bvsc(
        Path(args.jobg8),
        Path(args.geo_lookup),
    )
